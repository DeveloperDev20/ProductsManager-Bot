 ...
 execute: async(interaction) => {
    ...
    if (interaction.type == InteractionType.ApplicationCommand) {
      ...
      try {
        ...
        if(command.ownerOnly && interaction.user.id !== config.owner) return interaction.reply({content: "Solo los administradores pueden usar este comando.", ephemeral: true});
        if (command.cooldown) {
